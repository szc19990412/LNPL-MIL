...
class  ModelInterface(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        logits_ratio = results_dict['logits_ratio']

        #---->ratio loss
        ratio = torch.where(score>0.5)[0].shape[0]/self.topk
        if ratio < 0.25:
            ratio_label = torch.tensor([0]).cuda()
        elif ratio < 0.50:
            ratio_label = torch.tensor([1]).cuda()
        elif ratio < 0.75:
            ratio_label = torch.tensor([2]).cuda()
        else:
            ratio_label = torch.tensor([3]).cuda()

        #---->loss
        loss = self.loss(logits, label) + self.ratio*self.loss(logits_ratio, ratio_label)
